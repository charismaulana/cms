<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('email-recipients.update', $emailRecipient) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" id="name" name="name"
                                    value="{{ old('name', $emailRecipient->name) }}" required
                            ...
                            <div>
                                ...
                                <input type="email" id="email" name="email"
                                    value="{{ old('email', $emailRecipient->email) }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pertamina-blue focus:ring-pertamina-blue">
                                @error('email')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="flex items-center gap-6">
                                <div class="flex items-center">
                                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ $emailRecipient->is_active ? 'checked' : '' }}
                                ...
                                <div class="flex items-center">
                                    <input type="checkbox" id="is_global" name="is_global" value="1" {{ $emailRecipient->is_global ? 'checked' : '' }}
                                        class="rounded border-gray-300 text-pertamina-blue focus:ring-pertamina-blue"
                            ...
                            <div id="assignments-section" class="border-t pt-4">
                                ...
                                <div id="assignments-container">
                                    @forelse($emailRecipient->assignments as $index => $assignment)
                                        <div class="assignment-row flex gap-4 mb-3">
                                            <div class="flex-1">
                                                <select name="assignments[{{ $index }}][field]"
                                                    ...
                                                    @foreach($fieldOptions as $value => $label)
                                                        <option value="{{ $value }}" {{ $assignment->field == $value ? 'selected' : '' }}>{{ $label }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="flex-1">
                                                <select name="assignments[{{ $index }}][fungsi]"
                                                    class="block w-full rounded-md border-gray-300 shadow-sm focus:border-pertamina-blue focus:ring-pertamina-blue text-sm">
                                                    <option value="">All Fungsi</option>
                                                    @foreach($fungsiOptions as $value => $label)
                                                        <option value="{{ $value }}" {{ $assignment->fungsi == $value ? 'selected' : '' }}>{{ $label }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
    ...
    <script>
        let assignmentIndex = {{ $emailRecipient->assignments->count() > 0 ? $emailRecipient->assignments->count() : 1 }};
