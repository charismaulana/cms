<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('realizations.edit') }} - {{ $realization->contract->contract_number }}
        </h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <form method="POST" action="{{ route('realizations.update', $realization) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="month" name="month" required
                                    ...
                                    @foreach($months as $num => $name)
                                        <option value="{{ $num }}" {{ old('month', $realization->month) == $num ? 'selected' : '' }}>
                                            {{ $name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="year" name="year" required
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pertamina-blue focus:ring-pertamina-blue">
                                    @foreach($years as $y)
                                        <option value="{{ $y }}" {{ old('year', $realization->year) == $y ? 'selected' : '' }}>{{ $y }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('year')
                            ...
                            <div>
                                ...
                                <input type="text" id="pr_number" name="pr_number"
                                    value="{{ old('pr_number', $realization->pr_number) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pertamina-blue focus:ring-pertamina-blue">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="po_number" name="po_number"
                                    value="{{ old('po_number', $realization->po_number) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pertamina-blue focus:ring-pertamina-blue">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" id="sa_number" name="sa_number"
                                    value="{{ old('sa_number', $realization->sa_number) }}"
                            ...
                            <div>
                                ...
                                <input type="date" id="sa_date" name="sa_date"
                                    value="{{ old('sa_date', $realization->sa_date?->format('Y-m-d')) }}" readonly
                            ...
                            <div>
                                ...
                                <input type="number" id="realization_value" name="realization_value"
                                    value="{{ old('realization_value', $realization->realization_value) }}" required
                                    min="0" step="0.01"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pertamina-blue focus:ring-pertamina-blue">
                            </div>
                            ...
                            <div class="md:col-span-2">
                                ...
                                <textarea id="notes" name="notes" rows="3"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-pertamina-blue focus:ring-pertamina-blue">{{ old('notes', $realization->notes) }}</textarea>
                            </div>
                        </div>

                        <div class="mt-6 flex items-center justify-end space-x-4">
                            <a href="{{ route('contracts.show', $realization->contract) }}"
